...
const INIT_APP_INDEX = 1;
const NUMBER_OF_APPS = 10;
const NUMBER_OF_LIBS = 50;
...
function generateApp(appName) {
  const command = `nx g @nrwl/react:application ${appName} --compiler=swc --e2eTestRunner=none --routing --setParserOptionsProject --no-interactive`;
  cp.execSync(command);

  cp.execSync(
    `nx g @theunderscorer/nx-semantic-release:setup-project ${appName} --no-npm`
  );
...
function generateParentLib(appName, libName) {
  const command = `nx generate @nrwl/react:library ${libName} --directory=${appName} --appProject=${appName} --buildable --compiler=swc --setParserOptionsProject --standaloneConfig`;
  cp.execSync(command);
